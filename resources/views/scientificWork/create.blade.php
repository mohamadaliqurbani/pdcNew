    ...
    <div class="w-full px-3">
        <div class="bg-purple-100 w-4/5 mx-auto rounded-lg shadow-2xl">
            <header class="p-2 mb-2">
                <p class="text-center font-extrabold">فورم ثبت آثار علمی</p>
            </header>
            <div class="px-6 py-2 bg-white container">
                <form action="{{ route('scientific.store') }}" enctype="multipart/form-data" method="POST">
                    @csrf
                    ...
                    <textarea class="resize-none w-full form-input mb-2 @error('dicribesion') border border-red-500 @enderror"
                        id="dicribesion" name="dicribesion" value="{{ old('dicribesion') }}">
                        {{ old('dicribesion') }}
                    </textarea>
                    ...
                    <input type="date" class="w-full form-input mb-2 @error('relase_date') border border-red-500 @enderror"
                        id="relase_date" name="relase_date" value="{{ old('relase_date') }}">
                    ...
                    <input type="text" class="w-full form-input mb-2 @error('work_url') border border-red-500 @enderror"
                        id="work_url" name="work_url" value="{{ old('work_url') }}">
                    ...
                    <input type="file" class="w-full form-input mb-2 @error('file_work') border border-red-500 @enderror"
                        id="file_work" name="file_work" value="{{ old('file_work') }}">

                    @error('file_work')
                        <p class="text-red-500 font-semibold mb-1">{{ $message }}</p>
                    @enderror
                    <button class="bg-blue-500 hover:bg-blue-400 text-white p-3 rounded-lg mb-0">ثبت کردن</button>
                </form>
            </div>
        </div>
    </div>
